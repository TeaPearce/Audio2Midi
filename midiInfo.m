...
%    name = ['UNKNOWN META EVENT: ' num2str(type)]; dataStr =    ['nn=' num2str(data(1)) '  vel=' num2str(data(2))];
  end
...
% meta 0x21 = MIDI port number, length 1 (? perhaps)
  ...
  elseif (type==176); name = 'Controller Change';         len=2; dataStr = ['ctrl=' num2str(data(1)) '  value=' num2str(data(2))];  
  ...
  elseif (type==224); name = 'Pitch Bend';                len=2; 
    val = data(1)+ (data(2)*128);       % where data(2) is MSB
...
%    name = ['UNKNOWN MIDI EVENT: ' num2str(type)]; dataStr = num2str(data);
    name = ['UNKNOWN MIDI EVENT: ' num2str(type)]; dataStr =    ['1=' num2str(data(1)) '2=' num2str(data(2))];
  end
